...
function Reflect() {
  ...
  return <Flow><Steps active={1}/><div className={styles.analysis}><div className={styles.orbit}><span>R</span><i/><i/><i/></div><div className={styles.eyebrow}>Private account-wide analysis</div><h1>{labels[phase]}</h1><p>Reflex uses the Codex account already signed in on this Mac. Analysis runs in temporary read-only Codex threads; only structured findings return to this page.</p><Progress.Root value={[18, 42, 68, 88][phase]} className={styles.progress}><Progress.Track><Progress.Indicator/></Progress.Track></Progress.Root><small className={styles.analysisNote}>Large histories may take a few minutes.</small>{analysis.isError && <div className={styles.error}>{String(analysis.error)}</div>}</div></Flow>;
}
