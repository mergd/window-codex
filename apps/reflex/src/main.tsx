...
function Shell() { return <div className={styles.shell}><header><Link to="/" className={styles.brand}><span>R</span> Reflex</Link><div className={styles.powered}>Powered by <b>window.codex</b></div><a href="https://window-codex-docs.pages.dev">Docs ↗</a></header><Outlet/></div>; }
...
function Improve() {
  ...
  return <Flow><Steps active={2}/><div className={styles.improve}>{task ? <><div className={styles.liveBadge}><span/> Codex task {task.status}</div><h1>{task.title}</h1><div className={styles.taskStream}><small>LATEST UPDATE</small><p>{task.lastMessage ?? "Codex is starting the task…"}</p></div><div className={styles.actions}><Button className={styles.secondaryButton} onClick={() => void navigate({ to: "/findings" })}>Back to findings</Button><Button className={styles.dangerButton} onClick={() => state.provider?.request({ method: "tasks.cancel", params: { taskId: task.id } }).then(setTask)}>Cancel task</Button></div></> : <><div className={styles.eyebrow}>Make it durable</div><h1>{suggestion.title}</h1><p>{suggestion.rationale}</p><div className={styles.prompt}><small>EXACT PROMPT</small><p>{suggestion.proposedPrompt}</p></div><button className={styles.workspace} onClick={() => selectWorkspace.mutate()}><span>⌘</span><div><small>WORKSPACE</small><b>{workspace?.label ?? "Choose a workspace"}</b></div><i>›</i></button><div className={styles.notice}>The window.codex side panel will show this origin, workspace, and exact prompt before anything starts.</div><Button className={styles.primary} disabled={!workspace || start.isPending} onClick={() => start.mutate()}>{start.isPending ? "Waiting for confirmation…" : "Review and start task →"}</Button>{start.isError && <div className={styles.error}>{String(start.error)}</div>}</>}</div></Flow>;
}
