...
function Landing() {
  ...
  const connection = useQuery({ queryKey: ["provider-connection"], queryFn: async () => { const provider = state.provider ?? await getCodexProvider({ timeoutMs: 1200 }); state.provider = provider; return provider.request({ method: "provider.info", params: {} }); }, retry: false, staleTime: 0 });
  useEffect(() => { if (connection.data?.connected) void navigate({ to: "/threads", replace: true }); }, [connection.data?.connected, navigate]);
  const connect = useMutation({ mutationFn: async () => { const provider = state.provider ?? await getCodexProvider({ timeoutMs: 1200 }); await provider.request({ method: "connect", params: { protocolVersion: "0.1", scopes: ["threads:metadata", "events:subscribe"] } }); state.provider = provider; return provider; }, onSuccess: async () => { await Promise.all([queryClient.invalidateQueries({ queryKey: ["provider-profile"] }), queryClient.invalidateQueries({ queryKey: ["provider-connection"] })]); void navigate({ to: "/threads" }); }, onError: error => setError(error instanceof Error ? error.message : String(error)) });
  const opening = connection.isLoading || Boolean(connection.data?.connected) || connect.isPending;
  return <main className={styles.landing}><section><div className={styles.eyebrow}>A clearer view of how you work</div><h1>Your Codex history,<br/><em>turned into momentum.</em></h1><p>Reflex reviews your complete Codex history to find themes, repeated friction, and high-leverage improvements—without giving the page your transcripts.</p><Button className={styles.primary} onClick={() => connect.mutate()} disabled={opening}>{connection.data?.connected ? "Connected — opening history…" : opening ? "Checking Codemask…" : "Connect with Codemask"}</Button>{error && <div className={styles.error}>{error}<small>Reload this page if Codemask was recently updated.</small></div>}<div className={styles.trust}><span>◇ Uses your signed-in Codex plan</span><span>◇ No separate API key</span><span>◇ Your data stays local</span></div></section><aside className={styles.preview}><div className={styles.previewHead}><span>Complete history reflection</span><span className={styles.pill}>All conversations</span></div><div className={styles.score}><small>PRIMARY OPPORTUNITY</small><strong>Turn repeated setup work into a reusable workflow.</strong><p>Configuration and deployment steps recur across your recent Codex work.</p></div><div className={styles.miniGrid}><div><span>128</span><small>turns reviewed</small></div><div><span>6</span><small>clear themes</small></div><div><span>4h</span><small>potential saved</small></div></div><div className={styles.themeBars}><label>Automation <i style={{ width: "84%" }}/></label><label>Deployment <i style={{ width: "68%" }}/></label><label>Debugging <i style={{ width: "44%" }}/></label></div></aside></main>;
}
